    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                // User-Wunsch 2026-09-13 (macOS + iOS): Abspielen/Favorit/Gesehen UND der
                ...
                // tvOS hat schon sein eigenes, unverändertes Muster oben (nur
                // actionButtonsRow, Download bleibt dort unten).
                #if os(macOS) || os(iOS)
                actionButtonsRow
                ...
                // Genres) — macOS + iOS zeigen actionButtonsRow bereits weiter oben,
                // tvOS hat sein eigenes Muster oben (nur die Buttons, kein Download).
                // Hier unten bleibt für keine Plattform mehr ein zweites Vorkommen übrig.

                // User-Anfrage 2026-09-03, zurückgenommen 2026-09-04 ("könnte doch nützlich
                // sein"): Download-Button ist auf tvOS unten (Downloads-Tab-Pendant).
                // macOS + iOS zeigen ihn bereits weiter oben.
                #if os(tvOS)
                DownloadButtonRow(item: selectedItem, profile: pickedProfile)
                #endif
            }
