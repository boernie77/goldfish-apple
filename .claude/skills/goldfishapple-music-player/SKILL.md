---
name: goldfishapple-music-player
description: "Use when working on the GoldfishApple music player (AVPlayer mini bar, album/track lists, columns, sorting, search, shuffle, audiobook detection, offline)."
---

# goldfishapple-music-player

Der eigene Musik-Player (Mac + iOS): Mini-Leiste, Album-/Titel-Listen, Spaltenauswahl, Sortierung, Suche, Shuffle, Hörbuch-Erkennung, Offline-Modus. tvOS hat kein Musik-Modul.

## Harte Regeln

- Musik-Suche muss case- UND diakritika-unempfindlich sein (`range(of:options:[.caseInsensitive,.diacriticInsensitive])`) - `MusicLibraryView` filtert client-seitig, ein Server-Fix erreicht diesen Pfad nicht.
- `MusicColumnVisibility` ist eine `UserDefaults`-Allowlist; SwiftUI beobachtet sie nicht automatisch - jede View braucht ihren eigenen Refresh-State (`musicColumnsRefresh`).
- Neue Musik-Dateien (`MusicColumns.swift` etc.) brauchen `xcodegen generate` vor dem Build.
- `MusicPlayerEngine.play(...)` setzt `isShuffling` über den `shuffle:`-Parameter selbst - Aufrufer dürfen es nicht mehr manuell setzen.
- Hörbuch-Ausschluss gilt nur für Zufalls-/Shuffle-Pfade; bewusste Auswahl und sequentielles „Alle abspielen" bleiben ungefiltert.
- Spalten-/Listen-Umbauten sind `#if os(macOS)`-gegatet; iOS behält die kompakte gestapelte Zeile.
